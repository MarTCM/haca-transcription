...
\section{The Small Darija Model Alternative}

For resource-constrained environments (such as CPU-only execution or systems with limited VRAM), the pipeline supports a lightweight alternative configuration via the \code{--darija-model small} flag. 

When enabled, the routing engine swaps the large-v3-turbo base and LoRA adapter with the specialized \href{https://huggingface.co/ychafiqui/whisper-small-darija}{\code{ychafiqui/whisper-small-darija}} model. Because this is a fully fine-tuned model rather than a parameter-efficient adapter, it is loaded directly (with no base model required). Additionally, the default base model for non-Arabic chunks automatically degrades from \code{large-v3} to \code{small}, lowering the global memory and compute footprint while preserving reasonable transcription accuracy.
...
headline option, off by default. The Darija model type can be chosen using the
\code{--darija-model} flag (defaulting to \code{large}, or \code{small} for the
lightweight pipeline). The Darija LoRA is a mutually-exclusive
...
\code{--darija-model} & \code{darija\_model} & \code{large} & Choice: \code{large} (LoRA) or \code{small} (ychafiqui fine-tune).\\
