...
Cette structure permet de retrouver rapidement tout le contenu d'un mois ou d'un compte
...
gestion des protections anti-bot recentes.
...
meme si la liste de la chaine change.
...
\begin{description}[style=nextline, leftmargin=2.2em, labelindent=0.4em]
  \item[\texttt{--scan-limit N}]
    ...
    50). Utile pour les chaines tres actives ou seul le flux recent interesse.
  \item[\texttt{--js-runtimes}]
    Specifie les interpretes JavaScript disponibles (\texttt{nodejs}, \texttt{deno},
    \texttt{phantomjs}…) passes a yt-dlp pour le rendu des pages protegees. Sans ce
    parametre, yt-dlp choisit automatiquement le premier interprete disponible.
  \item[\texttt{--dry-run}]
...
{compte}/{annee}/{mois}/{titre}.mp3
\end{lstlisting}
Le titre est derive de la date de publication et du shortcode Instagram
(ex. \texttt{2m\_maroc/2026/07/2026-07-01\_CxY3z.mp3}) pour garantir l'unicite.
...
Les trois modules (\texttt{fetch\_youtube.py}, \texttt{fetch\_instagram.py} et
\texttt{\_media\_common.py}) sont couverts par une suite de \textbf{178 tests} executes
avec \textbf{pytest}. Les tests utilisent des bouchons (\emph{mocks}) pour yt-dlp,
instaloader et FFmpeg afin de rester reproductibles sans connexion reseau ni media reels.
Les cas couverts incluent la construction de l'arborescence, la deduplication, la gestion
des erreurs reseau, la re-authentification 2FA et les modes \texttt{--dry-run}.
