...
\textbf{Extraction audio robuste et impersonation.} Pour contourner le probleme des flux video-only HEVC (sans piste audio) servis par TikTok, le script utilise un selecteur de format adaptatif (\texttt{play/worst[vcodec\^{}=h264]/...}) et realise l'extraction audio en deux temps (telechargement video puis extraction via un sous-processus \texttt{ffmpeg}) plutot que d'utiliser le post-processeur par defaut de yt-dlp. De plus, l'utilisation de l'environnement virtuel du projet (\texttt{.venv}) permet d'exploiter \texttt{curl\_cffi} pour l'impersonation de navigateur, indispensable pour obtenir de TikTok des flux munis de pistes audio.
