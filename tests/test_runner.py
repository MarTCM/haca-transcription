...
def media_root(tmp_path):
    ...
    (d / "202406010900.mp3").write_bytes(b"\x00")
    return root
...
def test_srt_output_path_mirrors():
    p = runner.srt_output_path("al-oula/2024/06/01/202406010900.mp3", "out/srt")
    assert p == Path("out/srt/al-oula/2024/06/01/202406010900.srt")
...
def test_run_file_success_writes_mirrored_srt(media_root, tmp_path):
    ...
    res = runner.run_file(
        media_root, "al-oula/2024/06/01/202406010900.mp3", _cfg(out_dir),
        transcribe_fn=fake_transcribe, write_fn=fake_write,
    )
    assert res.status == STATUS_COMPLETED
    assert res.srt_path == "al-oula/2024/06/01/202406010900.srt"
    assert written["path"] == out_dir / "al-oula/2024/06/01/202406010900.srt"
...
def test_run_file_transcribe_exception_is_failed(media_root, tmp_path):
    ...
    res = runner.run_file(
        media_root, "al-oula/2024/06/01/202406010900.mp3", _cfg(tmp_path / "out"),
...
def test_run_file_skips_existing(media_root, tmp_path):
    out_dir = tmp_path / "out/srt"
    existing = out_dir / "al-oula/2024/06/01/202406010900.srt"
...
def test_run_file_overwrite_reprocesses(media_root, tmp_path):
    ...
    res = runner.run_file(
        media_root, "al-oula/2024/06/01/202406010900.mp3",
